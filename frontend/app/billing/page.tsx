...
export default function BillingPage() {
  ...
  const paid = payments.filter(p => p.status === '입금완료').reduce((sum, p) => sum + p.amount, 0);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {payments.map(payment => {
            ...
            const statusStyles: Record<string, { bg: string; text: string; shadow: string }> = {
              ...
              대기: { bg: 'rgba(240, 240, 240, 0.8)', text: '#888', shadow: 'rgba(0,0,0,0.05)' }
            };
            const style = statusStyles[payment.status] || statusStyles['대기'];
